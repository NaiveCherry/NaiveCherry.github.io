...
- 在文档流中行内元素不会独占一行，只占自身内容大小，自左向右水平排列，视口宽度不够时会自动换行，不支持设置高和宽。

#### 不在文档流的特点
- 块元素脱离文档流后不再独占页面一行，默认高度和宽度由内容撑开。<br/>
- 行内元素脱离文档流后会会变成块元素，可以设置高度宽度<br/>

总的来说，脱离文档流后，块元素和行内元素不再区分。
- 关于浮动详情可见[CSS的基础笔记3](https://naivecherry.github.io/CSS的基础笔记3)中关于浮动部分
...
> 边框还有一个用法，设置盒子高度宽度为0，通过去掉一边边框，设置三个边框显示，设置其中左右两边的边框颜色为透明，可以做出一个三角形。
...
### 垂直布局的外边距折叠/重叠问题
- 1.垂直方向的外边距会出现重叠效果，兄弟元素之间相邻垂直的外边距两个都为正数或负数时取绝对值大的值，一正一负取两者的和。
- 2.垂直方向相邻的父子元素中，子元素的上外边距会传递给父元素，这时候设置子元素外边距也等于设置父元素外边距，解决方法有1.通过定位设置位置解决,2.通过设置内边距解决，3.让两个元素不相邻，加边框、设置BFC、用伪类::buefor添加table空元素等方式让父子元素外边距线不重叠。
...
- table：设置元素为表格样式。
- table-cell：设置元素为单元格td（正常布局不使用）
> 单元格元素可以通过设置vertical-align（垂直排列）为middle达到子元素在单元格元素中垂直居中。
